...
void GatewayDataSolve(int rssi, uint8_t *rx_buffer, uint8_t rx_len)
{
    ...
    {
        sscanf((const char *) &rx_buffer[1], "G{%ld,%ld,%ld,%d,%d,%d}G", &Rx_message.Target_ID, &Rx_message.From_ID,
                &Rx_message.Payload_ID, &Rx_message.Counter, &Rx_message.Command, &Rx_message.Data);
